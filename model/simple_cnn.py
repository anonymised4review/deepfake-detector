"""
Just a simple CNN that directly reads on the audio file.
"""
...
class SimpleCNN():
    ...
    def __init__(self, params = {}):
        ...
        X = Input((int(self.params["sr"] * self.params["audio_slice"]), 2))
        Y = X

        for h, s, k in zip(self.params["hidden"], self.params["strides"], self.params["kernel"]):
            Y = Conv1D(h, kernel_size=k, strides=s, padding='same', activation=self.params["conv_activation"])(Y)
